...
"""Airfoil Optimisation Using Differential Evolution
   Code by A.Dhruv
"""
...
"""Vortex-Source Panel Method Solver"""

def panelmethod(xp,yp,alpha):
    M=len(xp)
...
Clinitial=panelmethod(xp,yp,alpha)
print Clinitial
...
while (genmax>count):
    for i in range(Np):
        ...
        xp,yp=NACA(trail[i][:],c,N)
        score=panelmethod(xp,yp,alpha)
        
        if(score>=Clinitial and score<=Cltarget and trail[i][0]>=0 and trail[i][1]>=0 and trail [i][2]>=0):
            naca=trail[i][:]
            Clinitial=score
    count=count+1
